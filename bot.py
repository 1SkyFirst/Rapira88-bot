...
def send_menu(chat_id, uid=None):
    kb = build_keyboard_two_per_row(list(menu_items.keys()),
                                    extra_last_row=(["⚙️ Рапира"] if is_admin(uid) else None))
...
def admin_panel(m):
    ...
    bot.send_message(m.chat.id, "🔧 Рапира:", reply_markup=kb)
...
def on_set(c):
    try:
        ...
        timestamp = now_moscow().strftime("%d.%m.%Y %H:%M")
...
print("✅ Бот запущен (время МСК, данные сохраняются в /data).")
